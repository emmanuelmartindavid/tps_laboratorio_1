...
int getalphabetic(char *pString, int len)
{
	...
	char option[len];
	if (pString != NULL && getString(option, sizeof(option)) == 0 && isAlphabetic(option, sizeof(option)) == 0 && strnlen(option, sizeof(option)) <= len)
	{
...
int utn_getAlphabeticDescription(char *pResult, char *message, char *ErrorMessage, int retry, int len, int min)
{
	...
	{
		printf(message);
		if ( isSpace(bufferString)!=-1 && getalphabetic(bufferString, sizeof(bufferString)) == 0
			&& strlen(bufferString) >= min
