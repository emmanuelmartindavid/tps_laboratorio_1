...
int main(void) {
	...
	do {
		showMainMenu();
		returnMainMenu = utn_getNumber(&option,
				"\t\t\t\t\t\t\t\t\t\tINGRESE OPCION.\n",
		...
		if (returnMainMenu == 0) {
			...
			case 2:
				printf("\t\t\t\t\t\t\t\t\t\tBAJA JUGADOR\n\n");
				returnsortPlayers= sortPerIdPlayers(arrayPlayer, LEN_ARRAYPLAYERS,
						arrayConfederation,
						LEN_ARRAYCONFEDERATION);
				returnListPlayers	=listPlayers(arrayPlayer, LEN_ARRAYPLAYERS,
						arrayConfederation,
						LEN_ARRAYCONFEDERATION);
			...
			case 4:
				...
				if (returnRegisterPlayer == OK ) {
					do {
						showReportMenu();
						returnReportMenu = utn_getNumber(&optionReports,
								"\t\t\t\t\t\tINGRESE OPCION.\n",
								"Opcion invalida", 1, 6, 3);
					...
					} while (optionReports != 6 || returnReportMenu != 0);
				} else {
					printf(
			...
			default:
				...
				returnExit=utn_getDescriptionExit(exit,
	...
	} while ((stricmp(exit, "si") != 0 && returnExit !=0 )|| returnMainMenu != 0  );
