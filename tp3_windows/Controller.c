							...
							if (getIdNationalTeamPLayer(pPlayer, &idNationalTeamPlayer) == SUCCESS)
							{
												...
												"INGESE NO PARA CANCELAR BAJA.\n", "ERROR. REINTENTE.\n", 3) == 0)
								{
									if (stricmp(confirmation, "si") == 0)
									{
										...
										if (ll_remove(pArrayListPlayer, indexListPlayer) == 0)
										{
											deletePlayer(pPlayer);
...
{
	int returncontrollerListCallUpPlayer = ERROR;
	int lenListPlayers;
	char country[30];
	int auxIdNationalTeam;
	sPlayer *pPlayer = NULL;
	...
	if (pArrayListPlayer != NULL && pArrayListNationalTeam != NULL)
	{
		...
		ll_deleteLinkedList(pArrayListPlayerAux);
		ll_deleteLinkedList(pArrayListNationalTeamAux);
		returncontrollerListCallUpPlayer = SUCCESS;
	}
	return returncontrollerListCallUpPlayer;
}
				...
				if (indexListPlayer != ERROR)
				{
					...
					if (pPlayer != NULL)
					{
						if (getIdNationalTeamPLayer(pPlayer, &idNationalTeamPlayer) == SUCCESS)
						{
							...
							{
								if (setIdNationalTeamPlayer(pPlayer, 0) == SUCCESS)
								{
			...
			if (lenArrayListPlayer > 0)
			{
				...
				for (int i = 0; i < lenArrayListPlayer; i++)
				{
					...
					if (pPlayer != NULL)
					{
						...
						{
							if (fprintf(pFile, "%d,%s,%d,%s,%s,%d\n", auxId, auxFullName, auxAge, auxPosition, auxNationality, auxIdNationalTeam) > 0)
							{
...
int controllerEditNationalTeam(LinkedList *pArrayListNationalTeam, int idNationalTeam, int condition)
{
	...
	if (pArrayListNationalTeam != NULL)
	{
		...
		if (pArrayListNationalTeamAux != NULL)
		{
			indexListNationalTeam = controllerFindNationalTeamById(pArrayListNationalTeamAux, idNationalTeam);
			if (indexListNationalTeam != ERROR)
			{
				pNationalTeam = (sNationalTeam*) ll_get(pArrayListNationalTeamAux, indexListNationalTeam);
				if (pNationalTeam != NULL)
				{
		...
		if (pFile != NULL)
		{
			...
			if (lenArrayListNationalTeam > 0)
			{
				...
				for (int i = 0; i < lenArrayListNationalTeam; i++)
				{
					...
					{
						if (fprintf(pFile, "%d,%s,%s,%d\n", auxId, auxCountry, auxConfederation, auxCalledUp) > 0)
						{
...
int controllerGetIncrementalIdPlayer(int *pCondition)
{
	...
	if (pCondition != NULL)
	{
		if (flag == 0)
		{
			if (controllerLoadIdplayerTextMode("IdJugadoresAutoincremental.csv", &idAutoincremental) == ERROR)
			{
				returnControllerGetIncrementalIdPlayer = ERROR;
			}
		}
		flag = 1;
		if (*pCondition == ADDPLAYER)
		{
			idAutoincremental++;
...
{
	int returnControllerLoadPlayersByConfederationFromBinary;
	int flagConfederationExistence;
	if (path != NULL && pArrayListPlayer != NULL && pArrayListNationalTeam != NULL && pConfederationRegister != NULL)
	{
		controllerValidateConfederationExistence(pArrayListPlayer, pArrayListNationalTeam, pConfederationRegister, &flagConfederationExistence);
		if (flagConfederationExistence == CALLUP)
		{
			...
			{
				returnControllerLoadPlayersByConfederationFromBinary = ERROR;
			}
		}
		else
		{
			returnControllerLoadPlayersByConfederationFromBinary = CALLOFF;
			remove(path);
		}
	}
...
int controllerListPLayersByConfederationFromBinaryData(char *path, LinkedList *pArrayListPlayer, LinkedList *pArrayListNationalTeam, char *pConfederationRegister)
{
	...
	if (path != NULL && pArrayListPlayer != NULL && pArrayListNationalTeam != NULL && pConfederationRegister != NULL)
	{
		...
		if (pArrayListPlayerAux != NULL && pArrayListNationalTeamAux != NULL)
		{
			returnLoadPlayerBinary = controllerLoadPlayersByConfederationFromBinary(path, pArrayListPlayerAux, pArrayListNationalTeamAux, pConfederationRegister);
			if (returnLoadPlayerBinary == SUCCESS)
			{
			...
			else if (returnLoadPlayerBinary == ERROR)
			{
				returnControllerListPLayersByConfederationFromBinaryData = ERROR;
			}
			else if (returnLoadPlayerBinary == CALLOFF)
			{
				returnControllerListPLayersByConfederationFromBinaryData = CALLOFF;
			}
			ll_deleteLinkedList(pArrayListPlayerAux);
			ll_deleteLinkedList(pArrayListNationalTeamAux);
		}
...
void controllerValidateConfederationExistence(LinkedList *pArrayListPlayer, LinkedList *pArrayListNationalTeam, char *pConfederationRegister, int *pFlag)
{
	...
	char auxConfederation[30];
	sPlayer *pPlayer = NULL;
	if (pArrayListPlayer != NULL && pConfederationRegister != NULL && pFlag != NULL)
	{
		LinkedList *pArrayListPlayerAux = ll_clone(pArrayListPlayer);
		LinkedList *pArrayListNationalTeamAux = ll_clone(pArrayListNationalTeam);
		if (pArrayListPlayerAux != NULL && pArrayListNationalTeamAux != NULL)
			...
			if (lenListPlayer > 0)
			{
				for (int i = 0; i < lenListPlayer; i++)
				{
					...
					if (pPlayer != NULL)
					{
						if (getIdNationalTeamPLayer(pPlayer, &auxIdPlayer) == SUCCESS)
						{
							if (controllerGetConfederation(pArrayListNationalTeamAux, auxIdPlayer, auxConfederation) == SUCCESS)
							{
								if (stricmp(auxConfederation, pConfederationRegister) == 0)
								{
									*pFlag = CALLUP;
